...
L=0.004  # La longueur du barreau
ne=50 # Nombre d'élement

dx = L/ne   #La longueur de chaque élément
# Maillage pour l'espace : Coordonnées des points
x = np.linspace(0, L-dx, ne)+0.5*dx  # [0.5*dx, 1.5*dx, 2.5*dx, ..., L-0.5 dx]

tfin = 0.1 # Temps final
...
for i in range(0, Nt-1):  
    ...
    # Calculer Tapres aux points de maille externes : point ne-1
...
# Méthode 1 Pour trouver t_1/2 : 
T_dernierElm=tableau_temps_Tx[:,-1]                              
dT_demi=(tableau_temps_Tx[:,-1][-1]-20)*0.5+20
for i in range(len(T_dernierElm)):
    if T_dernierElm[i]>dT_demi:
        t_demi_dT=t[i] # t_demi_dT=t_1/2
        break


D_flash= 0.138 *L**2/t_demi_dT

print ('D: incertitude relative  =', abs(D-D_flash)/D*100, '%')

'''

# Méthode 2 Pour trouver t_1/2 : 
    
a=tableau_temps_Tx[:,-1]

indice=min(range(len(a)), key=lambda i: abs(a[i]-dT_demi))
t[indice] # t[indice]=t_1/2

D_flash= 0.138 *L**2/t[indice]
print ('D: incertitude relative  =', abs(D-D_flash)/D*100, '%')
'''